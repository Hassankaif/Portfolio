  ...
  return (
    <div className="flex justify-between items-center h-16 max-w-[1240px] mx-auto px-4 text-black bg-gradient-to-r to-emerald-50 from-sky-400 m-6 ml-3 mr-3 rounded-2xl">
      ...
      <div onClick={handleNav} className="cursor-pointer md:hidden">
        {nav ? <AiOutlineClose size={30} /> : <img className="rounded-full w-16 h-16" src={NavLogo} alt="logo img" />}
      </div>
      ...
      <ul
        className={
          nav
            ? 'fixed left-0 top-0 w-[60%] h-full border-r border-gray-200 bg-gradient-to-r to-emerald-50 from-sky-100 ease-in-out duration-500 p-4'
            : 'fixed left-[-100%] top-0 ease-in-out duration-500'
        }
      >
        <img className="rounded-lg w-full h-16 mix-blend-multiply" src={logo} alt="logo img" />
